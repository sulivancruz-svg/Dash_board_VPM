...
export async function syncGoogleAdsDataSnapshot(): Promise<GoogleAdsSyncResult> {
  const creds = await getGoogleAdsCredentials();
  if (!creds) {
    throw new Error('Conta de servico nao configurada. Configure as credenciais primeiro.');
  }
  ...
  if (!privateKey || !developerToken || !customerId || !serviceAccountEmail) {
    throw new Error('Credenciais incompletas no arquivo salvo');
  }
